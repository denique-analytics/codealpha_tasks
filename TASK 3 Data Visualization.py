...
plt.bar(ratings, counts, width=0.6)
...
top_anime = df[df["Rating"] == highest_rating]

plt.figure(figsize=(10, 6))

plt.barh(top_anime["Title"], top_anime["Rating"])
...
# Visualization 3: Rating Percentage
rating_percentage = (
    df["Rating"]
...
ratings = rating_percentage.index.astype(str)
percentages = rating_percentage.values

plt.figure(figsize=(8, 5))

plt.bar(ratings, percentages, width=0.6)

plt.title("Percentage of Anime by Rating")
plt.xlabel("Rating")
plt.ylabel("Percentage (%)")

plt.show()

# Visualization 4: Rating Distribution Histogram
plt.figure(figsize=(8, 5))

plt.hist(
    df["Rating"],
    bins=[4.35, 4.45, 4.55, 4.65],
    edgecolor="black"
)
...
plt.xticks([4.4, 4.5, 4.6])

plt.show()

# Visualization Findings 
# 1. Most anime in the dataset have a rating of 4.5.
# 2. The rating distribution is highly concentrated between
#    4.4 and 4.6.
# 3. Four anime have the highest rating of 4.6.
# 4. Only two anime have the lowest rating of 4.4.
# 5. The small rating range shows very little variation within this dataset.